...
Reglas del formato: montos en pesos sin decimales; `subtotal_lista` es la suma de los tres rubros antes del descuento; `total` = `subtotal_lista` − descuentos; rubro que no aplica lleva `items: []` (o `sesiones: []`) y `subtotal: 0`; `sena_50` = 50% del total redondeado hacia arriba a la centena; los `producto` van con el nombre **exactamente como figura en este documento**, respetando mayúsculas y sin agregar tildes (ej. `Papas Clasicas`, `Box Cafe (Cafe + Tost)`, `Gaseosa`); un nombre que no esté en la carta invalida la salida.
